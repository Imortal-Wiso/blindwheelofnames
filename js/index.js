...
var COLORS = ["#787878", "#303030", "#787878", "#303030" ];
var PI = Math.PI;
var TAU = PI * 2;
...
var wheelFactory = function wheelFactory(mountElem) {
  ...
  function initEvents() {
    ...
    Object.keys(eventMap).map(function (type) {
      return domElement.addEventListener(type, eventMap[type]);
    });
  }
  ...
  function removeEvents() {
    ...
    Object.keys(eventMap).map(function (type) {
      return domElement.removeEventListener(type, eventMap[type]);
    });
  }
...
wheel.init({
  ...
  onWheelTick: function onWheelTick() {
  ...
  _.extend(Two, {
    ...
    Utils: _.extend(_, {
      ...
      applySvgAttributes: function (node, elem) {
        ...
        styles.visible =
          !(_.isUndefined(styles.display) && styles.display === "none") ||
          ...
          _.each(commands, function (command, i) {
            ...
            switch (lower) {
              ...
              case "a":
                ...
                var _x =
                  mx * Math.cos(xAxisRotation) + my * Math.sin(xAxisRotation);
                var _y =
                  -mx * Math.sin(xAxisRotation) + my * Math.cos(xAxisRotation);
                ...
                var cx =
                  _cx * Math.cos(xAxisRotation) -
                  _cy * Math.sin(xAxisRotation) +
                  (x1 + x4) / 2;
                var cy =
                  _cx * Math.sin(xAxisRotation) +
                  _cy * Math.cos(xAxisRotation) +
                  (y1 + y4) / 2;
      ...
      getCurveFromPoints: function (points, closed) {
        ...
        for (var i = 0; i < l; i++) {
          ...
          b.controls.left.x = _.isNumber(b.controls.left.x)
            ? b.controls.left.x
            : b.x;
          b.controls.left.y = _.isNumber(b.controls.left.y)
            ? b.controls.left.y
            : b.y;

          b.controls.right.x = _.isNumber(b.controls.right.x)
            ? b.controls.right.x
            : b.x;
          b.controls.right.y = _.isNumber(b.controls.right.y)
            ? b.controls.right.y
            : b.y;
        }
      },
      ...
      getControlPoints: function (a, b, c) {
        ...
        b.u = _.isObject(b.controls.left)
          ? b.controls.left
          : new Two.Vector(0, 0);
        b.v = _.isObject(b.controls.right)
          ? b.controls.right
          : new Two.Vector(0, 0);
  ...
  var canvas = {
        ...
        if (!clip && !parentClipped) {
          if (!canvas.isHidden.test(fill)) {
            if (fill._renderer && fill._renderer.offset) {
              ...
              ctx.save();
              ctx.translate(
                -toFixed(fill._renderer.offset.x),
          ...
          if (!canvas.isHidden.test(stroke)) {
            if (stroke._renderer && stroke._renderer.offset) {
              ...
              ctx.save();
              ctx.translate(
                -toFixed(stroke._renderer.offset.x),
    ...
    texture: {
      render: function (ctx) {
        ...
        if (
          !this._renderer.effect ||
          ((this._flagLoaded || this._flagImage) && this.loaded)
        ) {
...
(function (Two) {
    ...
    path: {
      updateCanvas: function (elem) {
        ...
        canvas.width = Math.max(
          Math.ceil(elem._renderer.rect.width * scale),
          1
        );
        canvas.height = Math.max(
          Math.ceil(elem._renderer.rect.height * scale),
          1
        );
        ...
        var flagTexture =
          ...
          (this._stroke instanceof Two.RadialGradient &&
            (this._stroke._flagSpread ||
              ...
              this._stroke._flagCenter ||
              this._stroke._flagFocal)) ||
          (this._texture instanceof Two.Texture &&
            this._texture._flagLoaded &&
  ...
  var Renderer = (Two[Two.Types.webgl] = function (options) {
    ...
    this.program.textureCoords = gl.getAttribLocation(
      this.program,
      "a_textureCoords"
    );
    ...
    MakeObservable: function (object) {
      ...
      Object.defineProperty(object, "vertices", {
        ...
        set: function (vertices) {
          ...
          this._collection = new Two.Utils.Collection(
            (vertices || []).slice(0)
          );
...
(function (Two) {
  ...
  var Line = (Two.Line = function (x1, y1, x2, y2) {
    ...
    Path.call(this, [new Two.Anchor(-w2, -h2), new Two.Anchor(w2, h2)]);
...
(function (Two) {
  ...
  _.extend(Two.Text, {
    ...
    MakeObservable: function (object) {
      ...
      _.each(
        Two.Text.Properties.slice(0, 12),
...
(function (Two) {
  ...
  _.extend(Texture.prototype, Two.Utils.Events, Two.Shape.prototype, {
    ...
    flagReset: function () {
      this._flagSrc = this._flagImage = this._flagLoaded = this._flagScale = this._flagOffset = false;
    ...
    _update: function () {
      ...
      if (this._texture.loaded) {
        ...
        if (this._playing && this._frameRate > 0) {
          ...
          duration =
            (1000 * (this._lastFrame - this._firstFrame)) / this._frameRate;
        ...
        var oy = height * Math.floor(this._index / cols) - (ih - height) / 2;
...
(function (Two) {
  ...
  _.extend(Group.prototype, Two.Shape.prototype, {
    ...
    getBoundingClientRect: function (shallow) {
      ...
      this.children.forEach(function (child) {
        if (
          /(linear-gradient|radial-gradient|gradient)/.test(
            child._renderer.type
          )
        ) {
          return;
        }
